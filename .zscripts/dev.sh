...
echo "Next.js dev server is running in background (PID: $DEV_PID). Supervising..."

# Self-healing supervisor: this script runs in a background subshell of
# start.sh (init tree), where spawned processes PERSIST for the container
# lifetime. (Processes spawned from agent tool calls are reaped by the
# sandbox between calls — the init tree is the only sanctioned home for a
# long-lived server.) If the dev server dies (OOM, crash), respawn it so
# the live preview always comes back within ~15s.
while true; do
  sleep 15
  if ! kill -0 "$DEV_PID" 2>/dev/null; then
    echo "[DEV] $(date '+%F %T') dev server (PID $DEV_PID) died — respawning..."
    bun run dev >> dev.log 2>&1 &
    DEV_PID=$!
    echo "[DEV] respawned as PID $DEV_PID"
    wait_for_service "localhost" "3000" "Next.js dev server respawn" 30 || true
  fi
done
